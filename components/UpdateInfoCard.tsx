...
const UpdateInfoCard = (props: IProps) => {
  ...
  return (
    <div className={"w-11/12 sm:w-5/6 md:w-3/4 lg:w-3/6 mx-auto select-none"}>
      <div className={classNames("rounded-xl shadow-2xl py-2 px-4 mt-4 bg-opacity-85 bg-zinc-900", "transform transition ease-in-out duration-200")}>

        <div className={"divide-y divide-zinc-500"}>
          <div className="flex flex-grow">
            <span
              className={"text-xl text-zinc-300 select-none"}>
              <span className={"text-zinc-500"}>#{leadingZeros(props.updateInfo.id, 4)} </span>
          ...
          <div className={classNames("pt-2 grid gap-2 grid-cols-4")}>
            ...
            <div className="flex gap-2">
              <Clock className={"text-zinc-300 h-5 w-5 flex-none"}/>
              <span className={"flex-grow text-zinc-300 truncate"}>{moment(props.updateInfo.endTime).diff(moment(props.updateInfo.startTime), "seconds")}s</span>
            </div>

            <div className="flex gap-2">
              <CalendarPlus className={"text-green-300 h-5 w-5 flex-none"}/>
              <span className={"flex-grow text-zinc-300 truncate"}>{props.updateInfo.newCount}</span>
            </div>

            <div className="flex gap-2 inline-block align-middle">
              <Replace className={"text-zinc-300 h-5 w-5 flex-none align-bottom bg-"}/>
              <span className={"flex-grow text-zinc-300 truncate"}>{props.updateInfo.updatedCount}</span>
            </div>

            <div className="flex gap-2 inline-block align-middle">
              <CalendarMinus className={"text-red-300 h-5 w-5 flex-none"}/>
              <span className={"flex-grow text-zinc-300 truncate"}>{props.updateInfo.removedCount}</span>
            </div>
