  ...
  return (
    <Layout title={"Updates"}>
      ...
      <div className={"py-2 flex sticky top-0 z-40 bg-gradient-to-r from-teal-600 via-indigo-600 to-teal-500 rounded-b-xl"}>
        ...
        <div className="absolute z-50 left-1/2 transform -translate-x-1/2 -translate-y-2">
          <div className="flex flex-grow justify-center">
            <div className="{/*bg-gradient-to-b to-teal-600 from-indigo-600*/} bg-opacity-30 bg-zinc-900 pt-2 pb-3 px-8 rounded-xl">
              <span className={"text-zinc-200 text-2xl font-semibold select-none"}>Updates</span>
            </div>
          </div>
        </div>
        ...
        <Link href={"/"}>
          <div className={"flex ml-2 px-2 py-1 gap-2 bg-opacity-50 rounded-md cursor-pointer hover:bg-teal-300 hover:bg-opacity-30 select-none transition transform duration-200"}>
            <ArrowLeftIcon className={"mt-1 h-5 w-5 text-zinc-200"} />
            <span className={"text-xl text-zinc-200 hidden lg:block"}>Back</span>
          </div>
        </Link>
        ...
        <div className={"flex flex-grow justify-end"}>
          ...
          <div onClick={openFilter} className="flex px-2 py-1 mr-2 gap-2 bg-opacity-50 rounded-md cursor-pointer hover:bg-teal-300 hover:bg-opacity-30 select-none transition transform duration-200" >
            <FilterIcon className={"mt-1 h-5 w-5 text-zinc-200"} />
            <span className={"text-xl text-zinc-200 hidden lg:block"}>Filter</span>

            <div ref={filterRef} className={"origin-top-right absolute mt-8 -ml-28 w-48 rounded-md bg-white dark:bg-zinc-700 focus:outline-none z-50"}>
              <Transition
                show={showFilter}
                as={Fragment}
      ...
      <div className={"flex flex-col flex-grow justify-center items-center min-h-screen"}>
          <div className="loader ease-linear rounded-full border-8 border-t-8 border-zinc-200 h-64 w-64"/>
      </div>
      }

      <div
        id={"scrollableDiv"}
        className="w-full overflow-y-scroll scroll-hidden bg-gradient-to-b from-zinc-900 to-slate-900"
          ...
          <div className={"flex flex-col pb-7 items-center"}>
            ...
            <InfiniteScroll
              ...
              loader={<div className={"flex w-11/12 sm:w-5/6 md:w-3/4 lg:w-3/6 mx-auto py-3 mt-3 bg-blue-300 rounded-xl bg-opacity-30 shadow-2xl"}>
                <span className={"w-full text-center text-zinc-200"}>Lade weitere update infos</span>
              </div>}
              endMessage={
                <div className={"w-11/12 sm:w-5/6 md:w-3/4 lg:w-3/6 mx-auto py-3 mt-3 bg-red-300 rounded-xl bg-opacity-30 shadow-2xl flex"}>
                  <span className={"w-full text-center text-zinc-200"}>Keine weiteren update infos Verfügbar</span>
                </div>
              }
