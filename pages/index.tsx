  ...
  const CourseCard = (props: { name: string }) => {
    ...
    return (
        <Link href={`/${props.name}`}>
          <div
              className={classNames("bg-gray-700 bg-opacity-85 rounded-xl p-2 cursor-pointer hover:bg-blueGray-500 select-none", courseList[props.name] && "border border-sky-300")}>
        ...
        <div className={"flex flex-grow justify-center items-center h-screen overflow-y-scroll"}>
          ...
          {!loading &&
            <div className="flex flex-col flex-grow items-center" style={{height: "100%"}}>
              <div className={"divide-gray-500 w-full divide-y md:w-3/4 w-2/4 px-2 md:px-0 mt-3"}>
